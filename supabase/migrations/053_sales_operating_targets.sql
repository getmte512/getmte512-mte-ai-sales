...
create index if not exists sales_operating_targets_history_idx on public.sales_operating_targets(period,metric,effective_from desc);
...
begin
 ...
 update public.sales_operating_targets set effective_to=p_effective_from-1 where period=p_period and metric=p_metric and effective_to is null;
 ...
 insert into public.audit_events(actor_id,event_type,entity_type,entity_id,details) values(p_actor_id,'sales_operating_target_set','sales_operating_target',v_id,jsonb_build_object('period',p_period,'metric',p_metric,'target_value',p_target_value,'effective_from',p_effective_from,'note',trim(p_note)));
 return v_id;
end$$;
