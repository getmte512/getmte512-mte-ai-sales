...
begin
  ...
  if p_status in ('invited','active') then
    ...
    if v_smoke_checked_at is null or v_smoke_checked_at < now()-interval '24 hours' or v_smoke_checked_at > now() then
      raise exception 'A fresh production smoke test recorded within 24 hours is required before pilot activation.';
    end if;

    select status,checked_at into v_recovery_status,v_recovery_checked_at from public.backup_recovery_drills order by checked_at desc limit 1;
    if v_recovery_status is distinct from 'passed' then raise exception 'A passed backup recovery drill is required before pilot activation.'; end if;
    if v_recovery_checked_at is null or v_recovery_checked_at < now()-interval '7 days' or v_recovery_checked_at > now() then
      raise exception 'A passed backup recovery drill within 7 days is required before pilot activation.';
    end if;
  ...
  values('pilot_account_status_updated','pilot_account',p_id::text,p_actor_id,jsonb_build_object(
    'status',p_status,
    'contact_id',v_account.contact_id,
    'feedback_notes',v_account.feedback_notes,
    'launch_smoke_checked_at',v_smoke_checked_at,
    'recovery_drill_checked_at',v_recovery_checked_at
  ));
