...
create or replace function public.apply_conversation_recommendation_task(
...
begin
  ...
  if v_rec.task_id is not null then
    return jsonb_build_object('task_id',v_rec.task_id,'already_applied',true);
  end if;

  insert into public.sales_tasks(contact_id,title,due_at,created_by)
  values(v_rec.contact_id,left('Buyer reply: '||v_rec.suggested_next_action,500),(now()+interval '1 day'),p_actor_id)
  returning id into v_task_id;

  update public.conversation_recommendations
  set task_id=v_task_id,task_applied_at=now(),updated_at=now()
  where id=v_rec.id;

  insert into public.audit_events(action,entity_type,entity_id,actor_id,metadata)
  values('conversation_recommendation_task_created','conversation_recommendation',v_rec.id::text,p_actor_id,
    jsonb_build_object('task_id',v_task_id,'contact_id',v_rec.contact_id,'reply_id',v_rec.reply_id));

  return jsonb_build_object('task_id',v_task_id,'already_applied',false);
end;
$$;

create or replace function public.apply_conversation_recommendation_pipeline(
  p_recommendation_id uuid,
  p_actor_id uuid
)
returns jsonb
...
begin
  ...
  if v_rec.pipeline_applied_at is not null then
    select id into v_pipeline_id from public.sales_pipeline where contact_id=v_rec.contact_id;
    return jsonb_build_object('pipeline_id',v_pipeline_id,'stage',v_rec.suggested_pipeline_stage,'already_applied',true);
  end if;

  insert into public.sales_pipeline(contact_id,stage,updated_by)
  values(v_rec.contact_id,v_rec.suggested_pipeline_stage,p_actor_id)
  on conflict(contact_id) do update set stage=excluded.stage,updated_by=excluded.updated_by,updated_at=now()
  returning id into v_pipeline_id;

  update public.conversation_recommendations
  set pipeline_applied_at=now(),updated_at=now()
  where id=v_rec.id;

  insert into public.audit_events(action,entity_type,entity_id,actor_id,metadata)
  values('conversation_recommendation_pipeline_applied','conversation_recommendation',v_rec.id::text,p_actor_id,
    jsonb_build_object('pipeline_id',v_pipeline_id,'contact_id',v_rec.contact_id,'reply_id',v_rec.reply_id,'stage',v_rec.suggested_pipeline_stage));
