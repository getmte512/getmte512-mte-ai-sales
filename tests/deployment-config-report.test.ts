...
const script=readFileSync("scripts/deployment-config-report.mjs","utf8");const preflight=readFileSync("scripts/deployment-preflight.mjs","utf8");const pkg=JSON.parse(readFileSync("package.json","utf8"));
describe("deployment configuration report",()=>{
 ...
 it("accepts either supported Shopify server authentication mode",()=>{for(const key of["SHOPIFY_SHOP_DOMAIN","SHOPIFY_STORE_DOMAIN","SHOPIFY_ADMIN_ACCESS_TOKEN","SHOPIFY_CLIENT_ID","SHOPIFY_CLIENT_SECRET"]){expect(script).toContain(key);expect(preflight).toContain(key)}expect(script).toContain("shopAuthConfigured");expect(preflight).toContain("clientCredentials")});
});
