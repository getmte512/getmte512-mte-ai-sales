...
Apply every Supabase migration in order through `042_outreach_delivery_intents.sql` before deploying the matching application build. Milestone 11 depends on migrations `035`–`041` for the discovery queue, search-run provenance, saved profiles, profile analytics, structured review reasons, and transactional provider-usage guardrails. Milestone 12 begins with migration `042`, which adds the immutable outreach delivery ledger and service-role-only preparation/confirmation functions. Do not deploy code that can mark approved outreach sent against a database missing migration `042`.
