...
type ScoreAdjustment = { contact_id:string; adjustment:number; reason:string; created_at:string };
...
export async function GET(request: Request) {
  try {
    ...
    let query = supabase.from("contact_crm").select("*").order("created_at", { ascending: false }).limit(250);
    if (search) {
    ...
    const contacts=data??[]; const ids=contacts.map(contact=>contact.id);
    const evidenceByContact=new Map<string,ResearchEvidence>(); const adjustmentByContact=new Map<string,ScoreAdjustment>();
    if(ids.length){
      const [{data:evidence,error:evidenceError},{data:adjustments,error:adjustmentError}]=await Promise.all([
        supabase.from("contact_research_evidence").select("contact_id,source_type,source_url,confidence,research_note,researched_at").in("contact_id",ids).order("researched_at",{ascending:false}),
        supabase.from("lead_score_adjustments").select("contact_id,adjustment,reason,created_at").in("contact_id",ids).order("created_at",{ascending:false}),
      ]);
      if(evidenceError)throw evidenceError;if(adjustmentError)throw adjustmentError;
      ...
      for(const row of (adjustments??[]) as ScoreAdjustment[])if(!adjustmentByContact.has(row.contact_id))adjustmentByContact.set(row.contact_id,row);
    }
    return NextResponse.json({ contacts: contacts.map((contact) => {
      const evidence=evidenceByContact.get(contact.id); const manual=adjustmentByContact.get(contact.id);
      ...
      const scored=scoreLead(enriched); const score=applyLeadScoreAdjustment(scored.score,manual?.adjustment);
      return {...enriched,...scored,score,tier:score>=70?"high":score>=45?"medium":"low",manual_score_adjustment:manual?.adjustment??null,manual_score_reason:manual?.reason??null,manual_score_adjusted_at:manual?.created_at??null};
    }) });
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    const {user}=await requireSales(); const body = researchSchema.parse(await request.json());
    ...
    const {id,...fields}=body; const researchNote=body.notes||`Buyer research saved from ${source.sourceType==="linkedin"?"LinkedIn":"the company website"}.`;
    const supabase=createAdminClient(); const {data,error}=await supabase.rpc("save_contact_research",{p_contact_id:id,p_actor_id:user.id,p_fields:fields,p_source_type:source.sourceType,p_source_url:source.sourceUrl,p_confidence:source.confidence,p_research_note:researchNote});
    if(error)throw error; return NextResponse.json({contact:{id},research:data});
  } catch (error) {
