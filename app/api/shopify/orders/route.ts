...
export async function GET(){try{
  await requireUser();const readiness=getShopifyReadiness(process.env);
...
}catch(error){const message=error instanceof Error?error.message:"Unable to preview Shopify orders.";return NextResponse.json({error:message==="UNAUTHORIZED"?"Sign in is required.":message},{status:message==="UNAUTHORIZED"?401:500});}}
