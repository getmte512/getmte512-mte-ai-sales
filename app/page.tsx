...
export default async function Home(){
  ...
  return <main><header className="topbar"><div><span className="eyebrow">MORE THAN ENERGY</span><h1>Retail Contact CRM</h1></div><div className="account-controls"><span className="scope-pill">{role} access</span><Link className="sign-out-button" href="/strategy">Daily Command Center</Link><Link className="sign-out-button" href="/strategy/goals">Goal Focus</Link><Link className="sign-out-button" href="/review">Operating Review</Link><Link className="sign-out-button" href="/prospects">Prospect Discovery</Link><Link className="sign-out-button" href="/delivery">Outbound Delivery</Link>{role==="admin"&&<Link className="sign-out-button" href="/launch">Launch Checklist</Link>}<form action={signOut}><button className="sign-out-button">Sign out</button></form></div></header><ContactImporter role={role}/></main>;
}
